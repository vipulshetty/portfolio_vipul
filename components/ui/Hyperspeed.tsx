...
const roadMarkings_fragment = `
  ...
  float shoulderLinesWidth = laneWidth * uShoulderLinesWidthPercentage;
  
  float brokenLines = step(1. - brokenLineWidth, fract(uv.x * uLanes));
  brokenLines *= step(uBrokenLinesLengthPercentage, fract(uv.y * 10.));
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    if (typeof mergedOptions.distortion === "string") {
      mergedOptions.distortion = distortions[mergedOptions.distortion] as Distortion;
    }
...
export const hyperspeedPresets: { [key: string]: Partial<HyperspeedOptions> } = {
  one: {
    onSpeedUp: () => { },
    onSlowDown: () => { },
    distortion: 'turbulentDistortion',
